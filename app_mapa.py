...
# --- MAPA ---
# Mantenemos todos los puntos en el mapa incluso si filtramos (seleccionamos el color o estilo)
mapa = folium.Map(location=[df_procesado['LATITUD_MAPA'].mean(), df_procesado['LONGITUD_MAPA'].mean()], zoom_start=8)
marker_cluster = MarkerCluster().add_to(mapa)

for _, fila in df_procesado.iterrows():
    # Estructura del Popup aprobada
    html_popup = f"""<div style="width:250px; font-family: sans-serif; font-size:11px;">
    <h4 style="margin:0; color:#0056b3;">{fila['CODIGO IDENTIFICADOR']}</h4>
    <hr style="margin:5px 0;">
    <b>Estado:</b> {fila.get('¿SITIO VANDALIZADO ?', 'N/A')}<br>
    <b>Site:</b> {fila.get('NOMBRE DE SITE', 'N/A')}</div>"""
    
    # Marcador condicional: si está seleccionado se resalta, si no, se muestra normal
...
if seleccion != "TODOS":
    ...
    if fotos:
        ...
        st.warning("No se encontraron fotos en '1.FOTOS'.")
